...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const isValidUrl = (string) => {
        try {
          new URL(string); // Try to create a new URL object
          return true; // Valid URL
        } catch (error) {
          return false; // Invalid URL
        }
      };
      // In the onSubmit method or where the product data is prepared:
      const productData = {
        productId: productId,
        sku: data.sku || "",
        ...
        slug: data.slug
          ? isValidUrl(data.slug)  // If the slug is a valid URL, use it as-is
            ? data.slug            // If it's a valid URL, don't change it
            : data.slug            // Else, fallback to the manual slug
          : data.title.toLowerCase().replace(/[^A-Z0-9]+/gi, "-"),  // If no slug, generate from title
  ...
  // Example of how you might use the updated handleProductSlug
  ...
  const handleProductSlug = (value) => {
    ...
    if (isValidUrl(value)) {
      ...
      // If it's not a valid URL, generate a slug from the title
      const generatedSlug = value.toLowerCase().replace(/[^A-Z0-9]+/gi, "-");
      setValue("slug", generatedSlug);
      setSlug(generatedSlug);
    }
  };

  // Utility function to check if a value is a valid URL
  const isValidUrl = (string) => {
    try {
      new URL(string);
      return true;  // Valid URL
